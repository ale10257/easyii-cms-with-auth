...
class AController extends Controller
{
    ...
    private function cropSlide($model)
    {
        $t = (new \yii\db\Query())
            ->select('value')
            ->from('easyii_settings')
            ->where(['name' => 'data_slider_on_main_page'])
            ->one();
         preg_match_all('|\d{2,4}|', $t['value'], $data);
        $model->image = Image::upload($model->image, 'carousel', $data[0][0], $data[0][1], true);
    }
}
