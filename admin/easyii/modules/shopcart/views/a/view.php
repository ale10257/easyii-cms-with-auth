...
<dl class="dl-horizontal shopcart-view">
    ...
    <dt>IP</dt>
    <dd><?= $order->ip ?> <a href="//freegeoip.net/?q=<?= $order->ip ?>" class="label label-info"
                             target="_blank">info</a></dd>
        ...
        <tr>
            ...
            <td>
                <?php if ($good->discount) : ?>
                    ...
                    <strike>
                        <small class="smooth"><?= $good->price ?></small>
                    </strike>
                <?php else : ?>
                    <b><?= $good->price ?></b>
                <?php endif; ?>
            </td>
            <td><a href="<?= Url::to(['/admin/' . $module . '/goods/delete', 'id' => $good->primaryKey]) ?>"
                   class="glyphicon glyphicon-remove confirm-delete" title="<?= Yii::t('easyii', 'Delete item') ?>"></a>
            </td>
        </tr>
    <?php endforeach; ?>
    </tbody>
...
<h2 class="text-right">
    <small><?= Yii::t('easyii/shopcart', 'Total') ?>:</small> <?= $order->cost ?></h2>
