...
<?= $this->render('_menu') ?>
    ...
    <dl class="dl-horizontal guestbook-view">
        ...
        <dt>IP</dt>
        <dd><?= $model->ip ?> <a href="//freegeoip.net/?q=<?= $model->ip ?>" class="label label-info" target="_blank">info</a>
        </dd>
    ...
    <hr>
    <h2>
        <small><?= Yii::t('easyii/guestbook', 'Answer') ?></small>
    </h2>
...
<?php if ($model->answer == '' && $model->email) : ?>
    <div class="checkbox">
        <label>
            <input type="checkbox" name="mailUser" value="1"
                   checked> <?= Yii::t('easyii/guestbook', 'Notify user about answer') ?>
        </label>
    </div>
<?php endif; ?>
