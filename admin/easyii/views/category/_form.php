...
<?php if ($settings['categoryThumb']) : ?>
    <?php if ($model->image) : ?>
        ...
        <a href="<?= Url::to(['/admin/' . $this->context->moduleName . '/a/clear-image', 'id' => $model->primaryKey]) ?>"
...
<? if (strpos(Yii::$app->request->getUrl(), 'photogallery/a/edit') !== false) : ?>
    <h3>Код для вставки галереи в статью</h3>
    <p><code>[gallery=<?= $model->slug ?>]</code></p><br>
<? endif ?>
