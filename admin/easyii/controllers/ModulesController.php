...
class ModulesController extends \yii\easyii\components\Controller
{
        ...
        if ($formModel->load(Yii::$app->request->post())) {
            if (Yii::$app->request->isAjax) {
                ...
                $newModule = new Module([
                    ...
                    'table_name' => json_encode($namesTables),
                ]);
                $a = $newModule->save();
                if ($a) {
                    ...
                    Yii::$app->db->createCommand("DROP TABLE  `$newTableName`;")->execute();
    ...
    public function actionDelete($id)
    {
        $model = Module::findOne($id);
        if ($model) {
            if ($data = $model->getAttributes(['name', 'table_name'])) {
                foreach (json_decode($data['table_name']) as $v) {
                    Yii::$app->db->createCommand("DROP TABLE IF EXISTS `$v`;")->execute();
                }
                $dir = Yii::getAlias('@app') . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . $data['name'];
